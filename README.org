...
As the author described the intended workflow is not really "Create a new activity and start work from there" but rather "this configuration seems to matter, let's save it". I do like the idea of starting a new activity (e.g. for a new project) from scratch, thus I have to setup a function that does that.
