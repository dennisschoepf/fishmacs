...
** Magit
Currently I use ~lazygit~ at work. Within emacs nothing beats ~magit~ though. ~forge~ is added for Github integration. A succint guide to setting it up can be found [[https://practical.li/spacemacs/source-control/forge-configuration/][here]]. Note: A ~classic~ token has to be generated for Github. It was also necessary to install to do the following on MacOS to set up GPG for to use encrypted ~.authinfo.gpg~:
